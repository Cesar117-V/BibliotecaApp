...
class _SeleccionLibrosScreenState extends State<SeleccionLibrosScreen> {
  ...
  Widget build(BuildContext context) {
    final librosFiltradosAgrupados = _librosAgrupados.entries.where((entry) {
      final tituloCoincide =
          entry.key.toLowerCase().contains(_busqueda.toLowerCase());
      final categoriaCoincide = _categoriaSeleccionada == 'Todas' ||
          entry.value.any((l) =>
              widget.mapCategorias[l.idCategoria] == _categoriaSeleccionada);
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text("Seleccionar Libros",
                                  ...
                                  child: Opacity(
                                    ...
                                    child: ListTile(
                                      ...
                                      subtitle: Column(
                                        ...
                                        children: [
                                          Text(
                                              'Autor: ${widget.mapAutores[libro.idAutor] ?? 'Desconocido'}'),
                                          Text(
                                              'Categoría: ${widget.mapCategorias[libro.idCategoria] ?? 'Sin categoría'}'),
                                          Text(
                                              libro.disponible == true
                                                  ? 'Disponible'
                                                  : 'No disponible',
                                              style: TextStyle(
                                                color: libro.disponible == true
                                                    ? Colors.green
                                                    : Colors.red,
                                              )),
                                        ],
                                            ...
                                            if (checked == true) {
                                              if (seleccionados.length <
                                                ...
                                                ScaffoldMessenger.of(context)
                                                  ...
                                                  const SnackBar(
                                                    content: Text(
                                                        'Ya no puedes seleccionar más libros'),
                                                    backgroundColor:
                                                        Colors.redAccent,
                                                  ),
                                                );
                  ...
                  onPressed: () {
                    if (seleccionados.length != widget.cantidadMaxima) {
                      ScaffoldMessenger.of(context).showSnackBar(
                        SnackBar(
                            content: Text(
                                'Debes seleccionar exactamente ${widget.cantidadMaxima} libros.')),
                      );
                      return;
                    }
